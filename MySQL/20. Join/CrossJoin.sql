...
-- Cross join multiple table
select * from products cross join colors cross join sizes;
select count(*) from products cross join colors cross join sizes;

-- Generate all possible product variations
explain select p.product_name, c.color_name, s.size_name, CONCAT(p.product_name, ' - ', c.color_name, ' - Size ', s.size_name)
 AS full_product_description from products p
cross join colors c
cross join sizes s
